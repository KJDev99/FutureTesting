...
async function profile() {
  ...
  function showmodal() {
    ...
    clearInterval(profile_interval);
    clearInterval(contact_interval);
    document.querySelector("#true_answers").textContent =
      "To'g'ri: " + true_answers;
...
async function contact() {
  ...
  for (let i = 0; i < json.questions.length; i++) {
    ...
    for (let j = 0; j < json.questions[i].variants.length; j++) {
      ...
      //     false_variants.push(div_answer);
      // }
      div_answer.style.backgroundColor = json.questions[i].variants[j].is_true
        ? "#0c0"
        : "";
